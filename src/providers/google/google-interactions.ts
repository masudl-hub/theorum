...
import { takeSsePayloads } from '../shared/sse.ts';
...
function withTap(req: ProviderCompleteRequest, transport: GeminiTransport): GeminiTransport {
  return {
    ...transport,
    fetch: tapFetch(req.tapUpstream, transport.fetch),
  };
}
...
async function fetchInteractionsResponse(
  ...
  if (!req.geminiBucket) {
    throw new TheorumError('geminiBucket is required for Interactions adapter');
  }
