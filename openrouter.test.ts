...
import { PUBLIC_UNAVAILABLE } from './error.ts';
import {
...
import type { ProviderCompleteRequest, TurnEvent } from './types.ts';
...
function mockStreamChunks(): string[] {
  ...
  const u = JSON.stringify({
    usage: { prompt_tokens: EXPECTED_INPUT_TOKENS, completion_tokens: EXPECTED_OUTPUT_TOKENS, total_tokens: EXPECTED_TOTAL_TOKENS },
  });
  return [
    `data: ${t1}\n\n`,
    `data: ${t2}\n\n`,
