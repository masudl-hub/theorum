...
async function* emitTurn(args: {
  ...
  while (currentAttempt <= maxRetries) {
    ...
    let latestStructured: unknown = undefined;
  ...
  if (!sawTokensEvent) {
    const inputChars = (safe.input.text?.length ?? 0) + (safe.input.fix?.artifact?.length ?? 0) + (system?.length ?? 800);
    let outputChars = 0;
    let thinkingChars = 0;
