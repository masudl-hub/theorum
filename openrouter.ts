import { tryStructured } from './delta.ts';
...
function applyToolCallObject(callObj: Record<string, unknown>, toolMap: Map<number, OpenRouterToolDelta>): void {
  let index = 0;
...
async function* streamOpenRouter(
  ...
  const apiKey = config.apiKey ?? (typeof Deno !== 'undefined' ? Deno.env.get('OPENROUTER_API_KEY') : undefined);
  if (!apiKey) {
