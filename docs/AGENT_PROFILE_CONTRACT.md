...
- `outputs.speech`: Pins for a speech-role profile (`voice`, `format: 'pcm' | 'mp3'`). The speech model itself is selected via `model.allow` / `model.config`. With `protocol: 'geminiInteractions'` / `provider: 'google'`, speech rides Google Interactions (`response_format: audio` + `speech_config`). With `protocol: 'openAi'` / `provider: 'openrouter'`, use `createProvider` (or `createSpeechProvider`) for `/audio/speech`.
