...
test('owns_except carves openrouter out of providers', async () => {
  ...
  const providersOwners = entriesForFile(
    graph,
    normalizePath('src/providers/openrouter.ts'),
    { ownsOnly: true },
  );
  assert.deepEqual(providersOwners.map(([id]) => id), ['openrouter']);

  const localOwners = entriesForFile(
    graph,
    normalizePath('src/providers/local.ts'),
    { ownsOnly: true },
  );
  assert.deepEqual(localOwners.map(([id]) => id), ['providers']);
});
