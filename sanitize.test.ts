...
import { sanitizeCsvText } from './attachments.ts';
import { CHAT_MEDIA_LIMITS } from './catalog.ts';
import { TheorumError } from './error.ts';
import { resolveTurn } from './resolve.ts';
import { PROJECT_ID_MAX, sanitizeProjectId, sanitizeText } from './sanitize.ts';
import { OMIT_INJECTION, OMIT_SENSITIVE } from './spans.ts';
...
Deno.test('more attachments than the profile allows are rejected', () => {
  ...
  assertThrows(() => resolveTurn({ profile: 'chat', input: { text: 'x', attachments: images } }), TheorumError);
});
